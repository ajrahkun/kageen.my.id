...
fetch('https://api.kageen.my.id/api/proxy?target=https://v5n7k8p2m4q9x1z.haiakuazra.workers.dev/').then(res => {
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    return res.json();
}).then(encryptedData => {
    const ciphertext = CryptoJS.enc.Base64.parse(encryptedData.result);
...
registerDirectoryBtn.addEventListener('click', () => {
    ...
    fetch('https://api.kageen.my.id/api/proxy?target=https://b9r3t7w4v8k2p5m.haiakuazra.workers.dev/').then(res => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        return res.json();
    }).then(encryptedData => {
        const ciphertext = CryptoJS.enc.Base64.parse(encryptedData.result);
...
fetch('https://api.kageen.my.id/api/proxy?target=https://x4m8q2z7v1k9p3b.haiakuazra.workers.dev/').then(res => {
...
fetch('https://api.kageen.my.id/api/proxy?target=https://7vn2kp9xl4mr8tb.haiakuazra.workers.dev/').then(res => {
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    return res.json();
}).then(encryptedData => {
    const secretKey = 'yK7mP2qW9xL4vN1zB6tF3sR8jQ5cX0dY2gH9vK4mP7zB2tF8sN';
...
async function x7m2k5v4p9q1r3z8b6t0w4v8k2p5m7r1x9z4m8q2z7v1k9p3b5() {
    ...
    while (!isRespondSuccess1) {
        try {
            ...
            if (result_1.success && result_1.result) {
                const data = q9z4k8p2m7v3r9b5t1w6x2v8k4p9m3r7x1z6m2q8z4v9k3p7b1(result_1.result);
    ...
    while (!isRespondSuccess12) {
        try {
            ...
            if (result_2.success && result_2.result) {
                const data = q9z4k8p2m7v3r9b5t1w6x2v8k4p9m3r7x1z6m2q8z4v9k3p7b1(result_2.result);
    ...
    listContainer.innerHTML = '<p style="color:#888; text-align:center; padding: 20px;">Memuat Data Peringkat ...</p>';
        ...
        for (let i = 0; i < targetHashtag.length; i++) {
            ...
            while (!isSuccess && retryCount < 5) {
                try {
                    const targetLink = encodeURIComponent(`https://api.kageen.my.id/api/direct2?query=${tag.query}`);
                    const proxyUrl = `https://api.kageen.my.id/api/proxy?target=${targetLink}`;

                    const res = await fetch(proxyUrl).then(r => r.json());

                    if (res.success && res.result) {
                        const data = q9z4k8p2m7v3r9b5t1w6x2v8k4p9m3r7x1z6m2q8z4v9k3p7b1(res.result);
...
async function v4m9k2p7q1z6r3b8t0w5x2v8k4p9m1r7x3z6m2q8z1v5k9p4b7() {
    ...
    try {
        const response = await fetch('https://api.kageen.my.id/api/proxy?target=https://w5n3r8x2m9p4v7k.haiakuazra.workers.dev/');
